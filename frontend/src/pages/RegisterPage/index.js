...
function RegisterPage() {
  ...
  const handleSubmit = (e) => {
    ...
    if (password !== confirmPassword) return toast.error("Passwords didn't match");
  ...
  useEffect(() => {
    document.title = 'Angel Fashion Studio | Register';
  }, []);
        ...
        <div className="w-full max-w-lg mx-auto lg:ml-auto lg:mr-0 bg-white/40 backdrop-blur-sm p-10 lg:p-14 border border-gold/10 shadow-2xl">
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="space-y-2">
              <label className="text-[9px] font-bold uppercase tracking-[0.4em] text-gold block">Email Address</label>
              <input
                type="email"
                name="email"
            ...
            {/* Password */}
            <div className="space-y-2">
              <label className="text-[9px] font-bold uppercase tracking-[0.4em] text-gold block">Password</label>
              <div className="relative">
                ...
                />
                <button type="button" onClick={() => setIsVisiblePassword(!isVisiblePassword)}
            ...
            {/* Confirm Password */}
            <div className="space-y-2">
              <label className="text-[9px] font-bold uppercase tracking-[0.4em] text-gold block">Confirm Password</label>
              <div className="relative">
                <input
                ...
                />
                <button type="button" onClick={() => setIsVisibleConfirmPassword(!isVisibleConfirmPassword)}
                  className="absolute right-0 top-1/2 -translate-y-1/2 text-bronze/40 hover:text-bronze transition-colors"
                  aria-label="Toggle confirm password visibility"
                >
                  <span className="material-symbols-outlined text-lg">
                    {isVisibleConfirmPassword ? 'visibility_off' : 'visibility'}
                  </span>
                </button>
              </div>
            </div>
            ...
            <p className="text-center text-[10px] font-bold uppercase tracking-[0.3em] text-bronze/50 pt-4">
              ...
              <Link to="/login" className="text-gold border-b border-gold/30 pb-0.5 hover:text-bronze hover:border-bronze transition-all">
                Login
              </Link>
            </p>
          </form>
